...
/**
 * App Providers Chapter Handler of BugReport.
 * @Author Tam.
 * Modifications :
 * 2017/4/17 : Created.
 */
public class AppProvidersHandler extends ModuleHandler {

    private final static String TAG = "APP PROVIDERS";

    public AppProvidersHandler(FileWrapper fileWrapper, String filter, int parseMode) {
        super(TAG, StatisticInfo.ChapterMapping.CHAPTER_APP_PROVIDERS, fileWrapper, filter, parseMode);
    }
    ...
    public void onModuleStart(String curLine, FileWrapper fileWrapper, BugReportParseEngine.ParseResult parseResult) throws IOException {
        ...
        do {
            ...
            if (line.startsWith(Consts.BUGREPORT_CHAPTER_APP_PROVIDERS_FILTER)) {
                if (!filterModuleStartFlag && filterModuleEndFlag) { // Filter module has started.
                    ...
                    filterModuleEndFlag = false;
                    // Record filter module end line.
                    mChapterFilterLineAtInfo.setChapterFilterStartLine(mCacheLinesStatistic.getCurrentLine() - 1);
                }
            }
        } while (line != null && StringUtils.kmp_indexOf(line, Consts.BUGREPORT_CHAPTER_APP_PROVIDERS_END) < 0);
