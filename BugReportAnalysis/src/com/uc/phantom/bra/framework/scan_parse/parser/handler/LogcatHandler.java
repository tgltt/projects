 ...
 * 2017/4/20 : Created.
 */
public class LogcatHandler extends ModuleHandler {

    private final static String TAG = "LOGCAT";

    public LogcatHandler(FileWrapper fileWrapper, String filter, int parseMode) {
        super(TAG, StatisticInfo.ChapterMapping.CHAPTER_LOGCAT, fileWrapper, filter, parseMode);
    }
    ...
    public void onModuleStart(String curLine, FileWrapper fileWrapper, BugReportParseEngine.ParseResult parseResult) throws IOException {
        ...
        StatisticInfo.PlainProblems filterProblems = parseResult.getFilterProblems();
        List<StatisticInfo.PlainProblems.Problem> listFilterException = filterProblems.getExceptionList();
        List<StatisticInfo.PlainProblems.Problem> listFilterFatal     = filterProblems.getFatalList();
        List<StatisticInfo.PlainProblems.Problem> listFilterBuildFingerprint = filterProblems.getBuildFingerprintList();
        List<StatisticInfo.PlainProblems.Problem> listFilterDexOpt    = filterProblems.getDexOptList();
            ...
            if (!filterModuleStartFlag && filterModuleEndFlag) {
                ...
                if (StringUtils.kmp_indexOf(line, Consts.PROBLEM_EXCEPTION_START) > -1 || StringUtils.kmp_indexOf(line, Consts.PROBLEM_ERROR_START) > -1) {
                    ...
                    if (StringUtils.kmp_indexOf(line, mFilter) > -1) {
                        listFilterException.add(problem);
                    }
                    ...
                    if (StringUtils.kmp_indexOf(line, mFilter) > -1) {
                        listFilterBuildFingerprint.add(problem);
                    }
